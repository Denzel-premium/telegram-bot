...
def start(msg):
    ...
    # Inline Buttons (Clean & Premium Look)
    ...
    inline.add(
        ...
        telebot.types.InlineKeyboardButton(
            "📂 Folder Passes", callback_data="folder_pass_menu"
        ),
    )
    ...
    if start_image:
        ...
        bot.send_message(
            msg.chat.id, caption_full, reply_markup=kb, parse_mode="Markdown"
        )
...
def generate_qr_handler(call):
    ...
    inline.add(
        telebot.types.InlineKeyboardButton(
            "💳 Upload Screenshot", callback_data="paid_main"
        )
    )
...
def show_demo_handler(call):
    ...
    if not demo_file and not demo_text:
        bot.send_message(
            chat_id,
            "❌ **Demo content currently available nahi hai.**",
            parse_mode="Markdown",
        )
        return
...
def paid_main_handler(call):
    ...
    bot.send_message(
        call.message.chat.id,
        "📸 **Payment hone ke baad yahan Screenshot bhejo.**",
        parse_mode="Markdown",
    )
...
def folder_pass_menu(call):
    ...
    for f in folders:
        ...
        status_text = (
            " ✅" if has_folder_access(user_id, f) else f" • ₹{f_price}"
        )
        inline.add(
            telebot.types.InlineKeyboardButton(
                f"📂 {f}{status_text}", callback_data=f"view_folder_{f}"
            )
        )
...
def view_folder_cb(call):
    ...
    if has_folder_access(user_id, folder):
        inline.add(
            telebot.types.InlineKeyboardButton(
                f"🔄 Re-fetch `{folder}`",
                callback_data=f"refetch_pass_{folder}",
            )
        )
        status_info = (
...
def paid_folder_prompt(call):
    ...
    bot.send_message(
        call.message.chat.id,
        f"📸 **Abhi payment ka Screenshot yahan bhejein (Folder: `{folder}`):**",
        parse_mode="Markdown",
    )
...
def refetch_pass_cb(call):
    ...
    if not has_folder_access(user_id, folder):
        bot.answer_callback_query(
            call.id,
            f"❌ Aapke paas '{folder}' ka active pass nahi hai!",
            show_alert=True,
        )
        return
...
def admin(msg):
    if msg.from_user.id != ADMIN_ID:
        bot.send_message(
            msg.chat.id, "❌ **Not allowed**", parse_mode="Markdown"
        )
        return
...
def setfolderprice(msg):
    ...
    if len(parts) < 3:
        bot.reply_to(
            msg,
            "❌ **Usage:** `/setfolderprice FOLDER_NAME PRICE`",
            parse_mode="Markdown",
        )
        return
    ...
    bot.reply_to(
        msg,
        f"✅ **Folder `{fname}` price set to ₹{fprice}**",
        parse_mode="Markdown",
    )
...
def setpasstext(msg):
    ...
    if not new_text:
        bot.reply_to(
            msg,
            "❌ **Usage:** `/setpasstext YOUR_CUSTOM_TEXT`",
            parse_mode="Markdown",
        )
        return
    ...
    bot.reply_to(
        msg,
        "✅ **Folder Pass instruction text updated successfully!**",
        parse_mode="Markdown",
    )
...
def handle_media(msg):
    ...
    if (
        user_id == ADMIN_ID
        and admin_states.get(ADMIN_ID) == "awaiting_demo_content"
    ):
        ...
        bot.reply_to(
            msg, "✅ **Demo updated successfully!**", parse_mode="Markdown"
        )
        return

    if (
        user_id == ADMIN_ID
        and admin_states.get(ADMIN_ID) == "awaiting_start_image"
        and msg.content_type == "photo"
    ):
        set_config("start_image", msg.photo[-1].file_id)
        admin_states[ADMIN_ID] = None
        bot.reply_to(
            msg, "✅ **Start Banner Photo updated!**", parse_mode="Markdown"
        )
        return
    ...
    if user_id == ADMIN_ID and msg.content_type == "video":
        if user_id not in current_folder:
            bot.reply_to(
                msg, "❌ **Use /setfolder first**", parse_mode="Markdown"
            )
            return
        folder = current_folder[user_id]
        add_video(folder, msg.video.file_id)
        bot.reply_to(
            msg, f"✅ **Saved in {folder}**", parse_mode="Markdown"
        )
        return
...
def requests(msg):
    ...
    for d in get_pending():
        ...
        f_price = (
            get_config(f"folder_price_{ptype}") or "49"
            if ptype != "ONLINE_VIP_PLAN"
            else get_config("price") or "29"
        )

        kb = telebot.types.InlineKeyboardMarkup()
        kb.add(
            telebot.types.InlineKeyboardButton(
                f"✅ Approve ({ptype})", callback_data=f"apv_{uid}_{ptype}"
            ),
            telebot.types.InlineKeyboardButton(
                "❌ Reject", callback_data=f"rej_{uid}"
            ),
        )
        bot.send_photo(
            msg.chat.id,
...
def approve(call):
    ...
    if ptype == "ONLINE_VIP_PLAN":
        ...
        bot.send_message(
            uid,
            "🎉 **VIP Online Plan Approved!\n📥 Click 'Download' button below to view content.**",
            parse_mode="Markdown",
        )
    else:
        ...
        if not vids:
            bot.send_message(
                uid,
                f"🎉 **Approved Pass for `{ptype}`! But folder is empty.**",
                parse_mode="Markdown",
            )
        else:
            bot.send_message(
                uid,
                f"🎉 **Approved Pass for `{ptype}`!\nSending videos now (Auto-Delete in 15 min, Re-fetch anytime)...**",
                parse_mode="Markdown",
            )
            ...
            bot.send_message(
                uid,
                f"⏳ **15 Minutes Timer Started for `{ptype}`!**\nVideos delete hone ke baad dobara Re-fetch karein:",
                reply_markup=inline,
                parse_mode="Markdown",
            )
...
def setdemo_cmd(msg):
    if msg.from_user.id == ADMIN_ID:
        ...
        bot.reply_to(
            msg,
            "🎬 **Abhi Demo Video/Photo text ke saath bhejo:**",
            parse_mode="Markdown",
        )
...
def setfolder(msg):
    if msg.from_user.id == ADMIN_ID:
        ...
        bot.reply_to(
            msg, f"📂 **Active folder: {name}**", parse_mode="Markdown"
        )
...
def delfolder(msg):
    if msg.from_user.id == ADMIN_ID:
        ...
        delete_folder(name)
        bot.reply_to(
            msg, f"🗑 **Deleted folder: {name}**", parse_mode="Markdown"
        )
    ...
    if msg.from_user.id == ADMIN_ID:
        ...
        if len(parts) >= 2:
            ...
            if folder:
                ...
                bot.reply_to(
                    msg, "❌ **Video deleted**", parse_mode="Markdown"
                )
...
def download(msg):
    ...
    for f in folders:
        inline.add(
            telebot.types.InlineKeyboardButton(
                f"📂 {f}", callback_data=f"open_{f}"
            )
        )

    bot.send_message(
        msg.chat.id,
        "📁 **Select a folder to view content online:**",
        reply_markup=inline,
        parse_mode="Markdown",
    )
...
def open_folder_cb(call):
    ...
    sent_videos[user_id] = []
    warning_caption = (
        "⚠️ *This video will auto-delete in 15 minutes! Save or watch soon.*"
    )
